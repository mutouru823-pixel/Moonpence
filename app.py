...
def _render_style_comparison(original_text: str, rewritten_text: str):
    try:
        ...
        st.caption("需安装 pandas 和 plotly 才能查看可视化对比")
...
def main():
                ...
                if not settings["api_key"]:
                    ...
                    with st.spinner("正在分析样本风格……"):
                        try:
                            ...
                            st.error(f"分析失败：{e}")
        with col_clear:
            if st.button("清空样本", use_container_width=True):
    ...
    if result_text:
        ...
        with col_actions:
            ...
            st.markdown("---")
            st.button("📋 复制结果", on_click=lambda: st.code(result_text, language=None))
            ...
            if st.button("🔄 根据建议再润色", type="primary", use_container_width=True):
                with st.spinner("正在根据反馈优化润色……"):
                    try:
                        ...
                        st.error(f"再润色失败：{e}")
        with col_clear:
            if st.button("清除评价", use_container_width=True):
